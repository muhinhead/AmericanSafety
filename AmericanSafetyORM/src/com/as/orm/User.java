...
public class User extends DbObject  {
    ...
    private Integer accManager = null;
    private Object avatar = null;
    private String url = null;

    public User(Connection connection) {
        super(connection, "user", "user_id");
        setColumnNames(new String[]{"user_id", "first_name", "last_name", "email", "login", "password", "updated_at", "created_at", "admin", "acc_manager", "avatar", "url"});
    }

    public User(Connection connection, Integer userId, String firstName, String lastName, String email, String login, String password, Timestamp updatedAt, Timestamp createdAt, Integer admin, Integer accManager, Object avatar, String url) {
        super(connection, "user", "user_id");
...
//        }
    ...
    public DbObject loadOnId(int id) throws SQLException, ForeignKeyViolationException {
        ...
        ResultSet rs = null;
        String stmt = "SELECT user_id,first_name,last_name,email,login,password,updated_at,created_at,admin,acc_manager,avatar,url FROM user WHERE user_id=" + id;
        try {
            ...
            if (rs.next()) {
                ...
                user.setAdmin(new Integer(rs.getInt(9)));
                user.setAccManager(new Integer(rs.getInt(10)));
                user.setAvatar(rs.getObject(11));
                user.setUrl(rs.getString(12));
                user.setNew(false);
            }
        } finally {
            try {
    ...
    protected void insert() throws SQLException, ForeignKeyViolationException {
         ...
         PreparedStatement ps = null;
         String stmt =
                "INSERT INTO user ("+(getUserId().intValue()!=0?"user_id,":"")+"first_name,last_name,email,login,password,updated_at,created_at,admin,acc_manager,avatar,url) values("+(getUserId().intValue()!=0?"?,":"")+"?,?,?,?,?,?,?,?,?,?,?)";
         try {
             ps = getConnection().prepareStatement(stmt);
             int n = 0;
    ...
    public void save() throws SQLException, ForeignKeyViolationException {
        if (isNew()) {
            ...
            String stmt =
                    "UPDATE user " +
                    "SET first_name = ?, last_name = ?, email = ?, login = ?, password = ?, updated_at = ?, created_at = ?, admin = ?, acc_manager = ?, avatar = ?, url = ?" + 
            ...
            try {
                ...
                ps.setObject(8, getAdmin());
                ps.setObject(9, getAccManager());
                ps.setObject(10, getAvatar());
                ps.setObject(11, getUrl());
                ps.execute();
            } finally {
                if (ps != null) ps.close();
            }
    ...
    public static DbObject[] load(Connection con,String whereCondition,String orderCondition) throws SQLException {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        String stmt = "SELECT user_id,first_name,last_name,email,login,password,updated_at,created_at,admin,acc_manager,avatar,url FROM user " +
        ...
        try {
            ...
            while (rs.next()) {
                DbObject dbObj;
                lst.add(dbObj=new User(con,new Integer(rs.getInt(1)),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getTimestamp(7),rs.getTimestamp(8),new Integer(rs.getInt(9)),new Integer(rs.getInt(10)),rs.getObject(11),rs.getString(12)));
                dbObj.setNew(false);
            }
        } finally {
            try {
    ...
    public Integer getAccManager() {
        return accManager;
    }

    public void setAccManager(Integer accManager) throws SQLException, ForeignKeyViolationException {
        if (null != accManager)
            accManager = accManager == 0 ? null : accManager;
        setWasChanged(this.accManager != null && !this.accManager.equals(accManager));
        this.accManager = accManager;
    }
    ...
    public Object[] getAsRow() {
        ...
        columnValues[8] = getAdmin();
        columnValues[9] = getAccManager();
        columnValues[10] = getAvatar();
        columnValues[11] = getUrl();
        return columnValues;
    }
    ...
    public void fillFromString(String row) throws ForeignKeyViolationException, SQLException {
        ...
        try {
            setAdmin(Integer.parseInt(flds[8]));
        } catch(NumberFormatException ne) {
            setAdmin(null);
        }
        try {
            setAccManager(Integer.parseInt(flds[9]));
        } catch(NumberFormatException ne) {
            setAccManager(null);
        }
        setAvatar(flds[10]);
